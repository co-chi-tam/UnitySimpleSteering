...
public class LOSManager : MonoBehaviour {
	...
	private bool completeUpdate = true;
		...
		if (losTexture != null) {
            ...
            if (!Application.isPlaying) {
                ...
                if (forceFullUpdate) {
                    ...
                    if (revealerCount == 0) {
                        Debug.Log("No LOSEntity items were marked as revealers! Tick the 'Is Revealed' checkbox for at least 1 item.");
						return;
					}
                }
...
//                }
			...
			bool isChanged = Application.isPlaying;
//            if (InterpolationRate > 0 && Application.isPlaying) {
//                if (lerpPixels == null) 
//					lerpPixels = pixels.ToArray();
//                else {
